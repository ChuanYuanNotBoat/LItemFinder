...
熔炉、漏斗等具有输入/输出语义的菜单放入 M5 兼容矩阵。任意模组容器是尽力支持项，不是本阶段
阻断条件。
...
- Screen 打开/关闭与客户端 tick 生命周期。
- 玩家背包槽排除、已知原版菜单 resolver。
- 内容指纹与仅变化时采集。
- 开发日志显示容器 ID、槽位数和采集原因，不打印物品敏感组件全文。

退出条件：支持矩阵中的基础容器不会混入玩家背包槽，不支持菜单不会被误保存。

### M3 — 快照、嵌套与持久化

- 创建根 `InventorySnapshot`。
- 解析潜影盒嵌套内容和稳定路径。
- 写入 `InMemoryStorageIndex` 与 `SqliteSnapshotRepository`。
- 切换 scope 和客户端退出时正确排空/关闭。

退出条件：修改、关闭、重开容器后只保留最新状态；重启客户端后搜索结果恢复。

### M4 — 查询调试面与可观察性

- 提供仅开发用途的命令或简易调试入口：精确查询、文本查询、索引统计、删除当前 scope 数据。
- 记录被跳过菜单和身份失败的原因计数。
- 日志可在不打开 debug 时保持安静。

退出条件：无需读取 SQLite 文件即可完成端到端验收；本阶段不制作正式 HUD。

### M5 — 兼容、打包与 Alpha 验收

- 完成原版兼容矩阵和至少两个常见模组容器的探索性测试。
- Windows/Linux Jar-in-Jar smoke test。
- CI 编译 NeoForge 模块并运行纯 Java 测试。
- 生成带 GPL-3.0 与第三方许可证的可安装 JAR。
- 更新用户安装、数据位置和卸载说明。
