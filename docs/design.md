...
- 紧凑库存总览、数量编辑与会话草稿（已实现首批）
- 精确 variant 来源分配与会话获取计划（已实现首批，不自动执行）
- 全屏容器地图和局部 3D Y 层投影视图（已实现首批）
- 搜索、设置、分组与诊断界面
- 可选导航服务接入后的路线渲染

独立 `navigation-core` 已建立纯 Java 搜索和前置任务提案原型；游戏世界观测与可安装导航 Mod
仍待实现，地图在服务缺席时仅显示示意停靠线。
